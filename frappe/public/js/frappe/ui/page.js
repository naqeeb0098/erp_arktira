...
frappe.ui.Page = class Page {
	...
	add_main_section() {
		...
		if (this.single_column) {
			...
			this.add_view(
				...
				`
				<div class="row layout-main m-0">
					<div class="layout-side-section layout" style="width:266px;"></div>
	...
	update_sidebar_icon() {
		...
		let sidebar_wrapper = this.wrapper.find(".layout-side-section");
		let is_sidebar_collapsed = sidebar_wrapper.hasClass("collapsed");
		sidebar_toggle_icon.html(
			frappe.utils.icon(
				is_sidebar_collapsed ? "es-line-sidebar-expand" : "es-line-sidebar-collapse",
				"md"
			)
		);
	}
